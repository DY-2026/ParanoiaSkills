...
- Added a transport-neutral read-only Application Service for status, routing, health, next action, Gate preview, graph export/inspection, record inspection, and workspace validation, with CLI parity regressions and no MCP SDK dependency.
- Added a `candidate` v1.4 MCP Boundary RFC for a local `stdio-first`, single-workspace, read-only adapter, including privacy filtering, forbidden tools, host compatibility evidence, Human Gate, exit gates, and rollback.
- Added `gamedesignos demo`, a zero-model, zero-key, installed-wheel-safe Golden Lighthouse flow that creates fresh public-synthetic evidence and a reviewed experiment, validates the workspace, and stops before the Commitment Human Gate.
- Added a runnable preview-first OpenAI-compatible reference host with an offline fixture, exact-request approval, semantic execution checkpoints, durable private response capture, bounded response validation, `outcome_unknown` recovery, and no automatic retry or persisted API key.
- Added a commit-pinned DeepSeek Harness engineering-fit audit that separates adopted execution-safety patterns, rejected runtime scope, current evidence, description cost, and rollback.
...
- Reused the package-level Lighthouse demo service from `scripts/create_golden_project.py`; fixture acceptance remains explicit and isolated to the checked-in golden-workspace generator.
...
- Corrected Project-Ready v1 status so accepted decisions, open Human Gates, and current default actions come from `01-decisions/DEC-*.json` instead of only the legacy decision log.
- Corrected the Chinese ten-minute guide so `ask` is consistently route-only unless a destination/workspace is explicit, and added regression coverage for `ask`/`start` onboarding parity.
